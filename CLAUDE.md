...
구현이 끝나면 **설명을 채팅에 늘어놓지 말고 `docs/phases/phase-N-<주제>.md` 로 남긴다.**
채팅에는 완료 사실과 문서 경로만 한두 줄로 알린다.
문서에는 설계 / 결정과 그 이유 / 트러블슈팅(문제→원인→해결→결과) / 검증 / 알려진 한계를 적는다.
`docs/phases/README.md` 의 목차와 상태도 함께 갱신한다.
**이 폴더는 gitignore 되어 있다 — 커밋하지 않는다.**
